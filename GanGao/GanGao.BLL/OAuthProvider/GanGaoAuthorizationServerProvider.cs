...
namespace GanGao.BLL.OAuthProvider
{
    ...
    public class GanGaoAuthorizationServerProvider : OAuthAuthorizationServerProvider
    {
        ...
        public override async Task GrantResourceOwnerCredentials(OAuthGrantResourceOwnerCredentialsContext context)
        {
            ...
            if(signResult.ResultType != Common.OperationResultType.Success)
            {
                context.SetError("invalid_grant", signResult.Message);
                return;
            }
            var user = await userService.FindUserAsync(context.UserName);
            if (user==null)
            {
                context.SetError("invalid_grant", "用户不存在");
                return;
            }            
            #endregion
